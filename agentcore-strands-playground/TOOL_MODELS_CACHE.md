...
python generate_tool_models_cache.py --output path/to/tool_models.json
```

This will:
1. Query Bedrock for all models in the region
2. Test each model for tool support (takes 1-2 minutes)
3. Save results to `tool_models.json`
...
- `tool_models.json` - Cache file (used by the front-end app)
- `app.py` - Front-end app that uses the cache
